...
while max(abs(grad_cons), abs(grad_dec)) >= 0.0000001:
  ...
  j+=1
  if j>0 and j%100 == 0 and j <10000:
    lr_var = learning_rate*np.exp(-j/250)

  # This is added to have a better comparison with other experiments
...
ax[0].set_ylabel('Local Minimum')
ax[0].legend()
ax[0].set_title('Parameter trajectory')
ax[0].grid()
...
for j in range(training_epochs):
  ...
  lr_var = min(float(lr_var * np.abs(grad_dec)), 1.2 * lr_var)  # cap at 1.2*lr
...
for j in range(training_epochs):
  ...
  # Adaptive Learning Rate depending on iteration
  lr_var1 = lr*np.exp(-j/training_epochs)
